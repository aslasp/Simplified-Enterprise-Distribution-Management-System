...
public class MgrExp extends StandardMgr {
	...
	JScrollPane jsp;
	String[][] content;
	String[] head = { "日期", "单据类型", "客户名", "商品名称", "型号", "数量", "销售单价", "销售总价" };
	JTextField fromDateField, toDateField;
	String fromDate = "0000-0-0", toDate = "9999-12-31";

	public MgrExp() {
		JButton addButton = new JButton("创建销售单", new ImageIcon(
				"GUI/icons/add.png"));
		...
		//
		try {
			content = Export.exportSho(fromDate, toDate);
		} catch (ClassNotFoundException | IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		expInfo = new JTable(content, head);
		expInfo.setVisible(true);
		jsp = new JScrollPane();
		jsp.setSize(jfWidth * 8 / 11, jfHeight * 3 / 5);
		jsp.setLocation(jfWidth / 32, jfHeight / 20);
		jsp.setViewportView(expInfo);
		this.add(jsp);
		//
		...
		class addyesbListener implements ActionListener {
			...
			public void actionPerformed(ActionEvent e) {
				if ((addcust.length() != 0) && (addname.length() != 0)
					...
					try {
						Export.exportAdd(addcust, addname, addmodel, addnum,
								addPrice);
					} catch (ClassNotFoundException | IOException e1) {
						// TODO Auto-generated catch block
						e1.printStackTrace();
					}
					JOptionPane.showMessageDialog(null, "销售单添加成功！", "添加成功",
							JOptionPane.INFORMATION_MESSAGE);
					jsp.remove(expInfo);
					MgrExp.this.remove(jsp);
					try {
						content = Export.exportSho(fromDate, toDate);
	...
	class delButtonListener implements ActionListener {
		...
		@Override
		public void actionPerformed(ActionEvent arg0) {
			selectedRowNum = expInfo.getSelectedRow();
			if (selectedRowNum != -1) {
				String[] delParameters = new String[4];
				delParameters[0] = (String) expInfo.getValueAt(selectedRowNum,
						0);
				delParameters[1] = (String) expInfo.getValueAt(selectedRowNum,
						2);
				delParameters[2] = (String) expInfo.getValueAt(selectedRowNum,
						3);
				delParameters[3] = (String) expInfo.getValueAt(selectedRowNum,
						4);
				try {
					Export.exportDel(delParameters[0], delParameters[1],
				...
				JOptionPane.showMessageDialog(null, "请先选择一张销售单！", "退货失败",
						JOptionPane.ERROR_MESSAGE);
			}
		}
